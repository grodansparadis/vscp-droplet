...
static void uart_initialize() {
  ...
  ESP_ERROR_CHECK(uart_set_pin(CONFIG_UART_PORT_NUM, CONFIG_UART_TX_IO,
                               CONFIG_UART_RX_IO, UART_PIN_NO_CHANGE,
                               UART_PIN_NO_CHANGE));
  ESP_ERROR_CHECK(uart_driver_install(CONFIG_UART_PORT_NUM, 8 * ESPNOW_DATA_LEN,
...
static void wifi_init() {
  ...
  ESP_ERROR_CHECK(
      esp_wifi_set_storage(WIFI_STORAGE_RAM));    // Config stored in ram
...
static void uart_read_task(void *arg) {
  ...
  for (;;) {
    size = uart_read_bytes(CONFIG_UART_PORT_NUM, data, ESPNOW_DATA_LEN,
                           pdMS_TO_TICKS(10));
    ESP_ERROR_CONTINUE(size <= 0, "");

    ret = espnow_send(ESPNOW_TYPE_DATA, ESPNOW_ADDR_BROADCAST, data, size,
                      &frame_head, portMAX_DELAY);
    ESP_ERROR_CONTINUE(ret != ESP_OK, "<%s> espnow_send", esp_err_to_name(ret));

    ESP_LOGI(TAG, "espnow_send, count: %d, size: %d, data: %s", count++, size,
             data);
...
static esp_err_t uart_write_handle(uint8_t *src_addr, void *data, size_t size, wifi_pkt_rx_ctrl_t *rx_ctrl) 
{
  ...
  ESP_LOGI(TAG, "espnow_recv, <%d> [" MACSTR "][%d][%d][%d]: %.*s", count++,
           MAC2STR(src_addr), rx_ctrl->channel, rx_ctrl->rssi, size, size,
           (char *)data);
...
void app_main() {
  ...
  esp_storage_init(); // Init tam storage (or nvs)
  ...
  /* uart initialization */
  uart_initialize();
  xTaskCreate(uart_read_task, "uart_read_task", 4 * 1024, NULL,
